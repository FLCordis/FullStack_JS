...
function createDeleteTransactionBtn(id){
    ...
    deleteBtn.addEventListener('click', async () => {
        ...
        const indexRemoval = transactions.findIndex((t) => t.id === id)
        transactions.splice(indexRemoval, 1)
        updateBalance()
    })
    return deleteBtn
}
...
async function saveTransaction(ev){
    ...
    if (id) {
        ...
        const indexRemoval = transactions.findIndex((t) => t.id === id)
        transactions.splice(indexRemoval, 1, transaction)
        document.querySelector(`#transation-${id}`).remove()
